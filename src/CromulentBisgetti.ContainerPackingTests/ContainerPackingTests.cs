			...
			using (Stream stream = assembly.GetManifestResourceStream(resourceName))
			{
				using (StreamReader reader = new StreamReader(stream))
				{
					...
					while (reader.ReadLine() != null && counter <= 700)
					{
						...
						List<Bin> containers = new List<Bin>();
						containers.Add(new Bin("0", Convert.ToDecimal(containerDims[0]), Convert.ToDecimal(containerDims[1]), Convert.ToDecimal(containerDims[2])));
